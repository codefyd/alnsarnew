...
function pgTeacherHizb(){
  mc('<div class="stitle"><i class="fas fa-book-quran"></i> عرض الحزب</div>'
    +'<div class="cc"><div class="chdr"><h3>عروض الحزب</h3><div class="cacts">'
    +'<button class="pb sm" onclick="openHizbPresentationForm()"><i class="fas fa-plus"></i> إضافة عرض</button>'
    +'<button class="ob sm" onclick="loadTeacherHizb()"><i class="fas fa-rotate"></i> تحديث</button>'
    +'</div></div><div class="cbody" id="hizbInfo"></div>'
    +'<div style="overflow:auto"><table class="dt"><thead><tr><th>التاريخ</th><th>الطالب</th><th>الحلقة</th><th>رقم الحزب</th><th>الدرجة</th><th>ملاحظات</th><th>إجراء</th></tr></thead><tbody id="hizbBody"></tbody></table></div></div>');
  loadTeacherHizb();
}
async function loadTeacherHizb(){
  await ensureAssessmentContext();
  var r=await api('جلب_عرض_الحزب',{الحلقة:'',التاريخ:todayISO()});
  D.hizbData=r;
  if(!r||!r.نجاح){document.getElementById('hizbInfo').innerHTML='<div class="empty"><h3>تعذر التحميل</h3><p>'+hesc((r&&r.خطأ)||'')+'</p></div>';return;}
  document.getElementById('hizbInfo').innerHTML='<div class="pgbar"><span>الفترة: <strong>'+hesc((r['الفترة']||{})['اسم_الفترة']||'—')+'</strong> · عدد عروض الحزب: '+arr(r['عروض']).length+'</span></div>';
  document.getElementById('hizbBody').innerHTML=arr(r['عروض']).map(function(x){return '<tr><td>'+hesc(String(x['تاريخ_العرض']||'').slice(0,10))+'</td><td><strong>'+hesc(x['اسم_الطالب']||'—')+'</strong><div style="font-size:11px;color:var(--ts)">'+hesc(x['رقم_الطالب']||'')+'</div></td><td>'+hesc(x['الحلقة']||'—')+'</td><td><span class="bp bp-in">'+hesc(x['رقم_الحزب']||'')+'</span></td><td>'+scoreBadge(x['الدرجة'],true)+'</td><td>'+hesc(x['ملاحظات']||'')+'</td><td><button class="erb sm" onclick="deleteHizbPresentation(\''+hesc(x['معرف'])+'\')"><i class="fas fa-trash"></i></button></td></tr>';}).join('')||'<tr><td colspan="7" style="text-align:center;padding:18px;color:var(--ts)">لا توجد عروض حزب</td></tr>';
}
function _fillHizbStudents(){
  var cc=val('hzCircle'), students=arr(window._hzStudents||[]);
  var opts=students.filter(function(s){return s['الحلقة']===cc || s.circle_name===cc;}).map(function(s){return '<option value="'+hesc(s['رقم_الطالب'])+'">'+hesc(s['اسم_الطالب'])+' — '+hesc(s['رقم_الطالب'])+'</option>';}).join('');
  var e=document.getElementById('hzStudent'); if(e)e.innerHTML=opts || '<option value="">لا يوجد طلاب في هذه الحلقة</option>';
}
async function openHizbPresentationForm(){
  await ensureAssessmentContext();
  var circles=_assessmentCircles(false);
  var allStudents=arr((D.hizbData||{})['طلاب']);
  if(!allStudents.length){var rr=await api('جلب_عرض_الحزب',{الحلقة:'',التاريخ:todayISO()});allStudents=arr(rr&&rr['طلاب']);}
  var available={}; allStudents.forEach(function(s){if(s['الحلقة'])available[s['الحلقة']]=true;});
  circles=circles.filter(function(c){return available[_circleName(c)];});
  if(!circles.length){Swal.fire({icon:'info',title:'لا توجد حلق متاحة',text:'لا توجد بيانات طلاب مرتبطة بالحلق.',confirmButtonColor:'#1a3c5e'});return;}
  var cOpts=circles.map(function(c){var n=_circleName(c);return '<option value="'+hesc(n)+'">'+hesc(n)+'</option>';}).join('');
  var res=await Swal.fire({title:'إضافة عرض حزب',html:'<div style="direction:rtl;text-align:right;display:grid;gap:10px"><div><label style="font-size:12px;font-weight:700">الحلقة</label><select id="hzCircle" class="swal2-select" onchange="_fillHizbStudents()" style="font-family:Tajawal;width:100%;margin:4px 0 0">'+cOpts+'</select></div><div><label style="font-size:12px;font-weight:700">الطالب</label><select id="hzStudent" class="swal2-select" style="font-family:Tajawal;width:100%;margin:4px 0 0"></select></div><div><label style="font-size:12px;font-weight:700">رقم الحزب</label><input id="hzNo" type="number" min="1" class="swal2-input" style="font-family:Tajawal;width:100%;margin:4px 0 0"></div><div><label style="font-size:12px;font-weight:700">الدرجة</label><input id="hzScore" type="number" min="0" max="10" step="0.25" class="swal2-input" style="font-family:Tajawal;width:100%;margin:4px 0 0"></div><div><label style="font-size:12px;font-weight:700">التاريخ</label><input id="hzDate" type="date" value="'+todayISO()+'" class="swal2-input" style="font-family:Tajawal;width:100%;margin:4px 0 0"></div><textarea id="hzNote" class="swal2-textarea" placeholder="ملاحظة اختيارية" style="font-family:Tajawal;direction:rtl;height:80px;width:100%;margin:0"></textarea></div>',didOpen:function(){window._hzStudents=allStudents;_fillHizbStudents();},showCancelButton:true,confirmButtonText:'حفظ',cancelButtonText:'إلغاء',confirmButtonColor:'#1a3c5e',customClass:{popup:'swal-rtl'},preConfirm:function(){if(!val('hzCircle')||!val('hzStudent')||!val('hzNo'))return Swal.showValidationMessage('الحلقة والطالب ورقم الحزب مطلوبة');return{رقم_الطالب:val('hzStudent'),رقم_الحزب:val('hzNo'),الدرجة:val('hzScore'),تاريخ_العرض:val('hzDate'),ملاحظات:val('hzNote')};}});
  if(!res.isConfirmed)return;
  spin(true,'جارٍ حفظ عرض الحزب…');var r=await api('حفظ_عرض_الحزب',res.value);spin(false);
  if(!r||!r.نجاح){Swal.fire({icon:'error',title:'تعذر الحفظ',text:(r&&r.خطأ)||'خطأ غير معروف',confirmButtonColor:'#1a3c5e'});return;}
  Swal.fire({icon:'success',title:'تم حفظ عرض الحزب',timer:1500,showConfirmButton:false});loadTeacherHizb();
}
...
// =====================================================================
// Phase 4D Fix 3 — التقييم حسب الصلاحيات + اختيار حلقة ثم طالب
// =====================================================================
async function ensureAssessmentContext(){
  if(D.assessmentContext && arr(D.assessmentContext['حلق']).length) return D.assessmentContext;
  var r=await api('جلب_سياق_التقييم_التعليمي',{});
  if(!r||!r.نجاح){
    // fallback لو لم يشغل المستخدم SQL بعد
    r={نجاح:true,إعدادات:D.eduAssessSettings||{},حلق:arr(D.setCircles).concat(arr(D.circles)).map(function(c){return typeof c==='string'?{اسم_الحلقة:c,مطلوب_ربط:true}:{اسم_الحلقة:c['اسم_الحلقة']||c.circle_name,مطلوب_ربط:true};})};
  }
  D.assessmentContext=r;
  D.eduAssessSettings=r['إعدادات']||D.eduAssessSettings||{};
  return r;
}
function _circleName(c){return typeof c==='string'?c:(c&& (c['اسم_الحلقة']||c.circle_name||c.name))||'';}
function _assessmentCircles(onlyRabt){
  var ctx=D.assessmentContext||{};
  var list=arr(ctx['حلق']).filter(function(c){return _circleName(c);});
  if(onlyRabt) list=list.filter(function(c){return c['مطلوب_ربط']===true || c['مفعل']===true || c.rabt_enabled===true;});
  var mine=teacherCircle();
  if(mine && list.some(function(c){return _circleName(c)===mine;})) list=list.filter(function(c){return _circleName(c)===mine;});
  var seen={};
  return list.filter(function(c){var n=_circleName(c); if(seen[n])return false; seen[n]=true; return true;});
}
function _toggleCircle(kind,name,multi){
  var key=kind+'Circles';
  var cur=arr(D[key]);
  if(!multi) cur=[name];
  else if(cur.indexOf(name)>=0) cur=cur.filter(function(x){return x!==name;});
  else cur.push(name);
  D[key]=cur;
  if(kind==='rabt') loadTeacherRabt();
  if(kind==='test') loadTeacherTests();
  if(kind==='hizb') loadTeacherHizb();
}
function _circleButtonsHtml(kind,circles,selected,multi){
  if(!circles.length) return '<div class="empty" style="padding:18px"><h3>لا توجد حلق متاحة</h3><p>تأكد من الصلاحيات أو تفعيل الحلق المطلوبة.</p></div>';
  return '<div style="display:flex;flex-wrap:wrap;gap:7px">'+circles.map(function(c){var n=_circleName(c);var on=selected.indexOf(n)>=0;return '<button class="'+(on?'pb':'ob')+' sm" onclick="_toggleCircle(\''+q(kind)+'\',\''+q(n)+'\','+(multi?'true':'false')+')"><i class="fas fa-mosque"></i> '+hesc(n)+'</button>';}).join('')+'</div>';
}
function _filterStudentsByCircles(students,circles){return arr(students).filter(function(s){return circles.indexOf(s['الحلقة'])>=0 || circles.indexOf(s.circle_name)>=0;});}
function _selectedRowsCodes(selector){var m={};document.querySelectorAll(selector).forEach(function(tr){m[tr.dataset.code]=true;});return m;}
function _renderStudentPicker(kind,students,rowSelector){
  var selected=_selectedRowsCodes(rowSelector);
  var opts=arr(students).filter(function(s){return !selected[s['رقم_الطالب']];}).map(function(s){return '<option value="'+hesc(s['رقم_الطالب'])+'">'+hesc(s['اسم_الطالب'])+' — '+hesc(s['الحلقة']||'')+'</option>';}).join('');
  var id=kind+'Pick';
  return '<div class="fb" style="margin-top:12px"><select id="'+id+'" class="fi" style="min-width:260px"><option value="">اختر الطالب…</option>'+opts+'</select><button class="pb sm" onclick="addAssessmentStudentRow(\''+kind+'\')"><i class="fas fa-plus"></i> إضافة الطالب</button></div>';
}
function addAssessmentStudentRow(kind){
  var id=kind+'Pick', code=val(id); if(!code)return;
  var list=kind==='rabt'?arr(D.rabtStudentsFiltered):arr(D.testStudentsFiltered);
  var s=list.find(function(x){return x['رقم_الطالب']===code;}); if(!s)return;
  if(kind==='rabt') document.getElementById('rabtBody').insertAdjacentHTML('beforeend',_rabtRowHtml(s,document.querySelectorAll('.rabtRow').length,Number((D.rabtData&&D.rabtData['الإعدادات']||{})['اقل_درجة_الربط']||7)));
  if(kind==='test') document.getElementById('testBody').insertAdjacentHTML('beforeend',_testRowHtml(s,document.querySelectorAll('.testRow').length,Number((D.testData&&D.testData['الإعدادات']||{})['اقل_درجة_الاختبار']||7)));
  var p=document.getElementById(kind+'PickerWrap'); if(p) p.innerHTML=_renderStudentPicker(kind,list,kind==='rabt'?'.rabtRow':'.testRow');
}
function _rabtRowHtml(s,i,pass){var score=s['الدرجة'];var passed=score===''||score==null?null:Number(score)>=pass;return '<tr class="rabtRow" data-code="'+hesc(s['رقم_الطالب']||'')+'"><td>'+(i+1)+'</td><td><strong>'+hesc(s['اسم_الطالب']||'—')+'</strong><div style="font-size:11px;color:var(--ts)">'+hesc(s['رقم_الطالب']||'')+' · '+hesc(s['الحلقة']||'')+'</div></td><td><input class="fi rabtScore" type="number" min="0" max="10" step="0.25" value="'+hesc(score==null?'':score)+'" style="width:100px" oninput="updateRabtRowStatus(this,'+pass+')"></td><td class="rabtStatus">'+scoreBadge(score,passed)+'</td><td><input class="fi rabtNote" value="'+hesc(s['ملاحظات']||'')+'" placeholder="اختياري" style="min-width:150px"></td><td><button class="erb sm" onclick="this.closest(\'tr\').remove();var p=document.getElementById(\'rabtPickerWrap\');if(p)p.innerHTML=_renderStudentPicker(\'rabt\',D.rabtStudentsFiltered,\'.rabtRow\')"><i class="fas fa-times"></i></button></td></tr>';}
function _testRowHtml(s,i,pass){var score=s['الدرجة'];var passed=score===''||score==null?null:Number(score)>=pass;return '<tr class="testRow" data-code="'+hesc(s['رقم_الطالب']||'')+'"><td>'+(i+1)+'</td><td><strong>'+hesc(s['اسم_الطالب']||'—')+'</strong><div style="font-size:11px;color:var(--ts)">'+hesc(s['رقم_الطالب']||'')+' · '+hesc(s['الحلقة']||'')+'</div></td><td><input class="fi testScore" type="number" min="0" max="10" step="0.25" value="'+hesc(score==null?'':score)+'" style="width:100px" oninput="updateTestRowStatus(this,'+pass+')"></td><td class="testStatus">'+scoreBadge(score,passed)+'</td><td><input class="fi testNote" value="'+hesc(s['ملاحظات']||'')+'" placeholder="اختياري" style="min-width:150px"></td><td><button class="erb sm" onclick="this.closest(\'tr\').remove();var p=document.getElementById(\'testPickerWrap\');if(p)p.innerHTML=_renderStudentPicker(\'test\',D.testStudentsFiltered,\'.testRow\')"><i class="fas fa-times"></i></button></td></tr>';}

function pgTeacherRabt(){
  var w=D.rabtWeek||currentWeekNo(), d=D.rabtDate||todayISO();
  mc('<div class="stitle"><i class="fas fa-link"></i> عرض الربط</div><div class="cc"><div class="chdr"><h3>إدخال درجات عرض الربط الأسبوعي</h3><div class="cacts"><input id="rabtWeek" class="fi" type="number" min="1" max="60" value="'+w+'" style="width:105px"><input id="rabtDate" class="fi" type="date" value="'+d+'"><button class="ob sm" onclick="loadTeacherRabt()"><i class="fas fa-rotate"></i> تحديث</button><button class="pb sm" onclick="saveTeacherRabt()"><i class="fas fa-floppy-disk"></i> حفظ الدرجات</button></div></div><div class="cbody" id="rabtInfo"><div class="empty"><div class="spri" style="margin:0 auto 10px"></div><p>جارٍ التحميل…</p></div></div><div class="cbody" id="rabtCircleBox"></div><div class="cbody" id="rabtPickerWrap"></div><div style="overflow:auto"><table class="dt"><thead><tr><th>#</th><th>الطالب</th><th>الدرجة من 10</th><th>الحالة</th><th>ملاحظة</th><th></th></tr></thead><tbody id="rabtBody"></tbody></table></div></div>');
  loadTeacherRabt();
}
async function loadTeacherRabt(){
  await ensureAssessmentContext();
  D.rabtWeek=Number(val('rabtWeek')||currentWeekNo());D.rabtDate=val('rabtDate')||todayISO();
  var circles=_assessmentCircles(true); if(!D.rabtCircles||!D.rabtCircles.length){var mine=teacherCircle();D.rabtCircles=mine?[mine]:(circles[0]?[_circleName(circles[0])]:[]);} D.rabtCircles=D.rabtCircles.filter(function(n){return circles.some(function(c){return _circleName(c)===n;});});
  document.getElementById('rabtCircleBox').innerHTML='<div style="font-size:12.5px;color:var(--ts);margin-bottom:8px">اختر حلقة أو أكثر من الحلق المطلوبة عليها الربط:</div>'+_circleButtonsHtml('rabt',circles,D.rabtCircles,true);
  var info=document.getElementById('rabtInfo'), body=document.getElementById('rabtBody');
  if(!D.rabtCircles.length){info.innerHTML='<div class="empty"><h3>لم يتم اختيار حلقة</h3><p>فعّل الحلق المطلوبة من إعدادات التقييم التعليمي ثم اختر الحلقة هنا.</p></div>';body.innerHTML='';document.getElementById('rabtPickerWrap').innerHTML='';return;}
  var r=await api('جلب_عرض_الربط',{الأسبوع:D.rabtWeek,التاريخ:D.rabtDate});D.rabtData=r;
  if(!r||!r.نجاح){info.innerHTML='<div class="empty"><h3>تعذر التحميل</h3><p>'+hesc((r&&r.خطأ)||'')+'</p></div>';return;}
  var set=r['الإعدادات']||{}, pass=Number(set['اقل_درجة_الربط']||7);
  D.rabtStudentsFiltered=_filterStudentsByCircles(arr(r['طلاب']),D.rabtCircles);
  info.innerHTML='<div class="pgbar"><span>الفترة: <strong>'+hesc((r['الفترة']||{})['اسم_الفترة']||'—')+'</strong> · الأسبوع '+hesc(r['الأسبوع'])+' · أقل درجة للنجاح: <strong>'+pass+'</strong> · الطلاب المتاحون: '+D.rabtStudentsFiltered.length+'</span></div>';
  var preset=D.rabtStudentsFiltered.filter(function(s){return s['الدرجة']!==null&&s['الدرجة']!==undefined&&s['الدرجة']!=='';});
  body.innerHTML=preset.map(function(s,i){return _rabtRowHtml(s,i,pass);}).join('')||'<tr><td colspan="6" style="text-align:center;padding:18px;color:var(--ts)">اختر طالبًا من الأعلى لإدخال الدرجة</td></tr>';
  if(!preset.length) body.innerHTML='';
  document.getElementById('rabtPickerWrap').innerHTML=_renderStudentPicker('rabt',D.rabtStudentsFiltered,'.rabtRow');
}
async function saveTeacherRabt(){
  var items=[];document.querySelectorAll('.rabtRow').forEach(function(tr){var score=tr.querySelector('.rabtScore').value;if(score==='')return;items.push({رقم_الطالب:tr.dataset.code,الدرجة:score,ملاحظات:tr.querySelector('.rabtNote').value});});
  spin(true,'جارٍ حفظ عرض الربط…');var r=await api('حفظ_عرض_الربط',{الحلقة:'',الأسبوع:Number(val('rabtWeek')||currentWeekNo()),التاريخ:val('rabtDate')||todayISO(),طلاب:items});spin(false);
  if(!r||!r.نجاح){Swal.fire({icon:'error',title:'تعذر الحفظ',text:(r&&r.خطأ)||'خطأ غير معروف',confirmButtonColor:'#1a3c5e'});return;}Swal.fire({icon:'success',title:'تم حفظ الدرجات',text:'تم حفظ '+(r['تم_الحفظ']||0)+' سجل',timer:1700,showConfirmButton:false});loadTeacherRabt();
}

function pgTeacherTests(){var d=D.testDate||todayISO();mc('<div class="stitle"><i class="fas fa-clipboard-list"></i> اختبارات الطلاب</div><div class="cc"><div class="chdr"><h3>إدخال درجات الاختبارات</h3><div class="cacts"><input id="testNo" class="fi" type="number" min="1" value="'+(D.testNo||1)+'" style="width:110px"><input id="testDate" class="fi" type="date" value="'+d+'"><button class="ob sm" onclick="loadTeacherTests()"><i class="fas fa-rotate"></i> تحديث</button><button class="pb sm" onclick="saveTeacherTests()"><i class="fas fa-floppy-disk"></i> حفظ الاختبار</button></div></div><div class="cbody" id="testInfo"></div><div class="cbody" id="testCircleBox"></div><div class="cbody" id="testPickerWrap"></div><div style="overflow:auto"><table class="dt"><thead><tr><th>#</th><th>الطالب</th><th>الدرجة من 10</th><th>الحالة</th><th>ملاحظة</th><th></th></tr></thead><tbody id="testBody"></tbody></table></div></div>');loadTeacherTests();}
async function loadTeacherTests(){
  await ensureAssessmentContext();D.testNo=Number(val('testNo')||1);D.testDate=val('testDate')||todayISO();
  var circles=_assessmentCircles(false); if(!D.testCircles||!D.testCircles.length){var mine=teacherCircle();D.testCircles=mine?[mine]:(circles[0]?[_circleName(circles[0])]:[]);} D.testCircles=D.testCircles.filter(function(n){return circles.some(function(c){return _circleName(c)===n;});});
  document.getElementById('testCircleBox').innerHTML='<div style="font-size:12.5px;color:var(--ts);margin-bottom:8px">اختر حلقة أو أكثر:</div>'+_circleButtonsHtml('test',circles,D.testCircles,true);
  var r=await api('جلب_اختبارات_الطلاب',{رقم_الاختبار:D.testNo,التاريخ:D.testDate});D.testData=r;
  if(!r||!r.نجاح){document.getElementById('testInfo').innerHTML='<div class="empty"><h3>تعذر التحميل</h3><p>'+hesc((r&&r.خطأ)||'')+'</p></div>';return;}
  var set=r['الإعدادات']||{}, pass=Number(set['اقل_درجة_الاختبار']||7), count=set['عدد_الاختبارات']||3;
  D.testStudentsFiltered=_filterStudentsByCircles(arr(r['طلاب']),D.testCircles);
  document.getElementById('testInfo').innerHTML='<div class="pgbar"><span>الفترة: <strong>'+hesc((r['الفترة']||{})['اسم_الفترة']||'—')+'</strong> · الاختبار '+hesc(r['رقم_الاختبار'])+' من '+hesc(count)+' · أقل درجة للنجاح: <strong>'+pass+'</strong> · الطلاب المتاحون: '+D.testStudentsFiltered.length+'</span></div>';
  var preset=D.testStudentsFiltered.filter(function(s){return s['الدرجة']!==null&&s['الدرجة']!==undefined&&s['الدرجة']!=='';});
  document.getElementById('testBody').innerHTML=preset.map(function(s,i){return _testRowHtml(s,i,pass);}).join('');
  document.getElementById('testPickerWrap').innerHTML=_renderStudentPicker('test',D.testStudentsFiltered,'.testRow');
}
async function saveTeacherTests(){var items=[];document.querySelectorAll('.testRow').forEach(function(tr){var score=tr.querySelector('.testScore').value;if(score==='')return;items.push({رقم_الطالب:tr.dataset.code,الدرجة:score,ملاحظات:tr.querySelector('.testNote').value});});spin(true,'جارٍ حفظ الاختبار…');var r=await api('حفظ_اختبارات_الطلاب',{الحلقة:'',رقم_الاختبار:Number(val('testNo')||1),التاريخ:val('testDate')||todayISO(),طلاب:items});spin(false);if(!r||!r.نجاح){Swal.fire({icon:'error',title:'تعذر الحفظ',text:(r&&r.خطأ)||'خطأ غير معروف',confirmButtonColor:'#1a3c5e'});return;}Swal.fire({icon:'success',title:'تم حفظ الاختبار',text:'تم حفظ '+(r['تم_الحفظ']||0)+' سجل',timer:1700,showConfirmButton:false});loadTeacherTests();}

function pgTeacherHizb(){mc('<div class="stitle"><i class="fas fa-book-quran"></i> عرض الحزب</div><div class="cc"><div class="chdr"><h3>عروض الحزب</h3><div class="cacts"><button class="pb sm" onclick="openHizbPresentationForm()"><i class="fas fa-plus"></i> إضافة عرض</button><button class="ob sm" onclick="loadTeacherHizb()"><i class="fas fa-rotate"></i> تحديث</button></div></div><div class="cbody" id="hizbInfo"></div><div class="cbody" id="hizbCircleBox"></div><div style="overflow:auto"><table class="dt"><thead><tr><th>التاريخ</th><th>الطالب</th><th>رقم الحزب</th><th>الدرجة</th><th>ملاحظات</th><th>إجراء</th></tr></thead><tbody id="hizbBody"></tbody></table></div></div>');loadTeacherHizb();}
async function loadTeacherHizb(){await ensureAssessmentContext();var circles=_assessmentCircles(false);if(!D.hizbCircle){var mine=teacherCircle();D.hizbCircle=mine||(_circleName(circles[0])||'');}document.getElementById('hizbCircleBox').innerHTML='<div style="font-size:12.5px;color:var(--ts);margin-bottom:8px">اختر الحلقة:</div>'+_circleButtonsHtml('hizb',circles,D.hizbCircle?[D.hizbCircle]:[],false);var r=await api('جلب_عرض_الحزب',{الحلقة:D.hizbCircle,التاريخ:todayISO()});D.hizbData=r;if(!r||!r.نجاح){document.getElementById('hizbInfo').innerHTML='<div class="empty"><h3>تعذر التحميل</h3><p>'+hesc((r&&r.خطأ)||'')+'</p></div>';return;}document.getElementById('hizbInfo').innerHTML='<div class="pgbar"><span>الفترة: <strong>'+hesc((r['الفترة']||{})['اسم_الفترة']||'—')+'</strong> · الحلقة: '+hesc(D.hizbCircle||'—')+' · عدد عروض الحزب: '+arr(r['عروض']).length+'</span></div>';document.getElementById('hizbBody').innerHTML=arr(r['عروض']).map(function(x){return '<tr><td>'+hesc(String(x['تاريخ_العرض']||'').slice(0,10))+'</td><td><strong>'+hesc(x['اسم_الطالب']||'—')+'</strong><div style="font-size:11px;color:var(--ts)">'+hesc(x['رقم_الطالب']||'')+'</div></td><td><span class="bp bp-in">'+hesc(x['رقم_الحزب']||'')+'</span></td><td>'+scoreBadge(x['الدرجة'],true)+'</td><td>'+hesc(x['ملاحظات']||'')+'</td><td><button class="erb sm" onclick="deleteHizbPresentation(\''+hesc(x['معرف'])+'\')"><i class="fas fa-trash"></i></button></td></tr>';}).join('')||'<tr><td colspan="6" style="text-align:center;padding:18px;color:var(--ts)">لا توجد عروض حزب</td></tr>';}
async function openHizbPresentationForm(){await ensureAssessmentContext();var circles=_assessmentCircles(false);var cOpts=circles.map(function(c){var n=_circleName(c);return '<option value="'+hesc(n)+'" '+(n===D.hizbCircle?'selected':'')+'>'+hesc(n)+'</option>';}).join('');var allStudents=arr((D.hizbData||{})['طلاب']);if(!allStudents.length){var rr=await api('جلب_عرض_الحزب',{التاريخ:todayISO()});allStudents=arr(rr&&rr['طلاب']);}var studentsJson=encodeURIComponent(JSON.stringify(allStudents));var res=await Swal.fire({title:'إضافة عرض حزب',html:'<div style="direction:rtl;text-align:right;display:grid;gap:10px"><div><label style="font-size:12px;font-weight:700">الحلقة</label><select id="hzCircle" class="swal2-select" onchange="window._hzStudents=JSON.parse(decodeURIComponent(\''+studentsJson+'\'));var cc=this.value;var opts=window._hzStudents.filter(function(s){return s[\'الحلقة\']===cc;}).map(function(s){return \'<option value=&quot;\'+hesc(s[\'رقم_الطالب\'])+\'&quot;>\'+hesc(s[\'اسم_الطالب\'])+\' — \'+hesc(s[\'رقم_الطالب\'])+\'</option>\';}).join(\'\');document.getElementById(\'hzStudent\').innerHTML=opts;" style="font-family:Tajawal;width:100%;margin:4px 0 0">'+cOpts+'</select></div><div><label style="font-size:12px;font-weight:700">الطالب</label><select id="hzStudent" class="swal2-select" style="font-family:Tajawal;width:100%;margin:4px 0 0"></select></div><div><label style="font-size:12px;font-weight:700">رقم الحزب</label><input id="hzNo" type="number" min="1" class="swal2-input" style="font-family:Tajawal;width:100%;margin:4px 0 0"></div><div><label style="font-size:12px;font-weight:700">الدرجة</label><input id="hzScore" type="number" min="0" max="10" step="0.25" class="swal2-input" style="font-family:Tajawal;width:100%;margin:4px 0 0"></div><div><label style="font-size:12px;font-weight:700">التاريخ</label><input id="hzDate" type="date" value="'+todayISO()+'" class="swal2-input" style="font-family:Tajawal;width:100%;margin:4px 0 0"></div><textarea id="hzNote" class="swal2-textarea" placeholder="ملاحظة اختيارية" style="font-family:Tajawal;direction:rtl;height:80px;width:100%;margin:0"></textarea></div>',didOpen:function(){window._hzStudents=allStudents;var e=document.getElementById('hzCircle');if(e)e.dispatchEvent(new Event('change'));},showCancelButton:true,confirmButtonText:'حفظ',cancelButtonText:'إلغاء',confirmButtonColor:'#1a3c5e',customClass:{popup:'swal-rtl'},preConfirm:function(){if(!val('hzCircle')||!val('hzStudent')||!val('hzNo'))return Swal.showValidationMessage('الحلقة والطالب ورقم الحزب مطلوبة');D.hizbCircle=val('hzCircle');return{رقم_الطالب:val('hzStudent'),رقم_الحزب:val('hzNo'),الدرجة:val('hzScore'),تاريخ_العرض:val('hzDate'),ملاحظات:val('hzNote')};}});if(!res.isConfirmed)return;spin(true,'جارٍ حفظ عرض الحزب…');var r=await api('حفظ_عرض_الحزب',res.value);spin(false);if(!r||!r.نجاح){Swal.fire({icon:'error',title:'تعذر الحفظ',text:(r&&r.خطأ)||'خطأ غير معروف',confirmButtonColor:'#1a3c5e'});return;}Swal.fire({icon:'success',title:'تم حفظ عرض الحزب',timer:1500,showConfirmButton:false});loadTeacherHizb();}

// =====================================================================
// Phase 4D Fix 4 — عرض الحزب بدون أزرار حلق خارجية
// =====================================================================
function pgTeacherHizb(){
  mc('<div class="stitle"><i class="fas fa-book-quran"></i> عرض الحزب</div>'
    +'<div class="cc"><div class="chdr"><h3>عروض الحزب</h3><div class="cacts">'
...
// =====================================================================
// Phase 4F — تقرير المعلم حسب الفترة والأسابيع
// =====================================================================
function tRepNum(v){return Number(v||0).toLocaleString('ar-SA');}
function tRepPct(v){return '<span class="bp bp-in">'+hesc(v||0)+'%</span>';}
async function pgTeacherFullReport(){
  mc('<div class="stitle"><i class="fas fa-chart-pie"></i> تقرير المعلم</div>'
    +'<div class="cc"><div class="chdr"><h3>تقرير الحلقة حسب الفترة النشطة</h3><div class="cacts">'
    +'<button class="ob sm" onclick="loadTeacherFullReport()"><i class="fas fa-rotate"></i> تحديث</button>'
    +'<button class="ob sm" onclick="exportTeacherFullReportCsv()"><i class="fas fa-file-csv"></i> CSV</button>'
    +'</div></div>'
    +'<div class="cbody" id="teacherFullSummary"><div class="empty"><div class="spri"></div><h3>جارٍ تحميل التقرير…</h3></div></div>'
    +'<div class="stabs"><button class="stab active" onclick="swST(this,\'teacherFullWeeks\')"><i class="fas fa-calendar-week"></i> الأسابيع</button><button class="stab" onclick="swST(this,\'teacherFullStudents\')"><i class="fas fa-users"></i> الطلاب</button></div>'
    +'<div class="spanel active" id="teacherFullWeeks"><div style="overflow:auto;direction:rtl"><table class="dt" style="direction:rtl;text-align:right"><thead><tr><th>الأسبوع</th><th>الفترة</th><th>حضور</th><th>تأخر</th><th>غياب</th><th>غياب بعذر</th><th>أتم</th><th>لم يتم</th><th>نسبة الإتمام</th></tr></thead><tbody id="teacherFullWeeksBody"></tbody></table></div></div>'
    +'<div class="spanel" id="teacherFullStudents"><div style="overflow:auto;direction:rtl"><table class="dt" style="direction:rtl;text-align:right"><thead><tr><th>#</th><th>الطالب</th><th>أيام التحضير</th><th>حضور</th><th>تأخر</th><th>غياب</th><th>نسبة الحضور</th><th>نسبة الإتمام</th></tr></thead><tbody id="teacherFullStudentsBody"></tbody></table></div></div>'
    +'</div>');
  await loadTeacherFullReport();
}
async function loadTeacherFullReport(){
  var r=await api('جلب_تقرير_المعلم_للفترة',{الحلقة:teacherCircle()});
  D.teacherFullReport=r;
  if(!r||!r.نجاح){document.getElementById('teacherFullSummary').innerHTML='<div class="empty"><h3>تعذر التحميل</h3><p>'+hesc((r&&r.خطأ)||'خطأ غير معروف')+'</p></div>';return;}
  var m=r.ملخص||{}, term=r.الفترة||{};
  document.getElementById('teacherFullSummary').innerHTML='<div class="pgbar" style="margin-bottom:12px"><span>الفترة: <strong>'+hesc(term['اسم_الفترة']||'—')+'</strong> · الحلقة: <strong>'+hesc(r['الحلقة']||teacherCircle()||'—')+'</strong> · '+hesc(term['تاريخ_البداية']||'')+' إلى '+hesc(term['تاريخ_النهاية']||'')+'</span></div>'
    +'<div class="kpi-grid">'
    +kpiCard('عدد الطلاب',m['عدد_الطلاب']||0,'fa-users','')
    ...
    +kpiCard('تأخر',m['تأخر']||0,'fa-clock','kpi-wa')
    +kpiCard('غياب',m['غياب']||0,'fa-user-xmark','kpi-er')
    +kpiCard('أتم الحفظ',m['أتم_الحفظ']||0,'fa-book-open','kpi-ok')+'</div>';
  document.getElementById('teacherFullWeeksBody').innerHTML=arr(r['أسابيع']).map(function(w){return '<tr><td><strong>الأسبوع '+tRepNum(w['رقم_الأسبوع'])+'</strong></td><td>'+hesc(w['بداية_الأسبوع']||'')+' - '+hesc(w['نهاية_الأسبوع']||'')+'</td><td>'+tRepNum(w['حضور'])+'</td><td>'+tRepNum(w['تأخر'])+'</td><td>'+tRepNum(w['غياب'])+'</td><td>'+tRepNum(w['غياب_بعذر'])+'</td><td>'+tRepNum(w['أتم_الحفظ'])+'</td><td>'+tRepNum(w['لم_يتم_الحفظ'])+'</td><td>'+tRepPct(w['نسبة_إتمام_الحفظ'])+'</td></tr>';}).join('')||'<tr><td colspan="9" style="text-align:center;padding:18px;color:var(--ts)">لا توجد بيانات</td></tr>';
  document.getElementById('teacherFullStudentsBody').innerHTML=arr(r['طلاب']).map(function(s,i){return '<tr><td>'+(i+1)+'</td><td><strong>'+hesc(s['اسم_الطالب']||'—')+'</strong><div style="font-size:11px;color:var(--ts)">'+hesc(s['رقم_الطالب']||'')+'</div></td><td>'+tRepNum(s['أيام_التحضير'])+'</td><td>'+tRepNum(s['حضور'])+'</td><td>'+tRepNum(s['تأخر'])+'</td><td>'+tRepNum(s['غياب'])+'</td><td>'+tRepPct(s['نسبة_الحضور'])+'</td><td>'+tRepPct(s['نسبة_إتمام_الحفظ'])+'</td></tr>';}).join('')||'<tr><td colspan="8" style="text-align:center;padding:18px;color:var(--ts)">لا توجد بيانات</td></tr>';
}
function exportTeacherFullReportCsv(){
  var rows=arr((D.teacherFullReport||{})['طلاب']);
  if(!rows.length){Swal.fire({icon:'info',title:'لا توجد بيانات للتصدير',confirmButtonColor:'#1a3c5e'});return;}
  ...
  var csv=cols.join(',')+'\n'+rows.map(function(x){return cols.map(function(c){return '"'+String(x[c]==null?'':x[c]).replace(/"/g,'""')+'"';}).join(',');}).join('\n');
  var b=new Blob(['\uFEFF'+csv],{type:'text/csv;charset=utf-8'});var a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='teacher_term_report_'+Date.now()+'.csv';a.click();
}
